...
declare const top_inflow: string;
let points: TGeo[] = JSON.parse(top_inflow);
...
htmlelem_year.addEventListener('change', async (e: Event) => {
  ...
  const points = (await getjson<TGeo>(url)) as TGeo[];
  set_doughnut(points, conf);

  fill_text<TGeo>('number-first', points, 'number', '');
  fill_text<TGeo>('name-first', points, 'name', '');
  fill_image<TGeo>('img-first', points, 'iso_2', 'name');
